...
void scan()
{
	...
	if (found)
	{
		...
		file.close();
		status = md5(openFileName) + " Infected";
	}
	else
	{
		file.open(result.c_str(), std::ios_base::app);
		...
		else if (!std::filesystem::is_directory(entry->path()) && entry->path().has_extension())
		{
			if (entry->path().extension() == ".exe")
			{
				...
				if (found)
				{
					...
					file.close();
					status = entry->path().string().c_str() + ' ' + md5(entry->path().string()) + " Infected ";
				...
				else
				{
					...
					file.close();
					status = entry->path().string().c_str() + ' ' + md5(entry->path().string()) + " Clean ";
